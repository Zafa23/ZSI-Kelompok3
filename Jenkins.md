...
![2.png](_resourc2-1.png)
> Selanjutnya membuat reverse proxy dan memberi certbot pertama buatlah file di gateway `cenginx`dengan type configurasi atau kalian bisa salin saja `sudo nano jenkins.conf` jika sudah ikuti langkah langkah pasang cert bot lewat documentasion ini [Certbot](http/certbot.eff.oinstructions?ws=nginx&os=ubuntufocal)


![3.png](_resourc3-1.png)
...
>  `cat .sid_rsa` selanjutnya kalian tempel di dalam sini dan isi seperti di gambar 


![8.png](_resourc8-1.png)
...
```
def branch = "main"
def repo = "http/github.cZafawayshub-frontend.git"
def cred = "apserver"
    ...
    stages {
        ...
        stage('Push to Docker Hub') {
            steps {
                sshagent([cred]) {
                    sh """ssh -o StrictHostKeyChecking=no ${server} << EOF
                        docker tag ${imagename}:latest ${dockerusernam${imagename}:latest
                        docker login -u ${dockerusername} -p DOCKER_PASSWORD
                        docker push ${dockerusernam${imagename}:latest
                        exit
                        EOF
                    """
                }
...
>Note : Disini saya membuat juga versi penjelasan dari kode yang saya buat berikut link nya [Frontend](http/github.cZafaZSI-KelompoblmaPenjelasan-Kode-Frontend.md)


![10.png](_resourc10-1.png)
...
![11.png](_resourc11-1.png)
> Disini masukan seperti di gambar saja 




![12.png](_resourc12-1.png)
...
```
def branch = "main"
def repo = "http/github.cZafawayshub-backend.git"
def cred = "appserver"
